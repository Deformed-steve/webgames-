(() => {
  ...
  class e {
    constructor(e) {
      ...
      })), this.gp.achievements.on("unlock", (e => this.trigger("CallAchievementsUnlock", e))), this.gp.achievements.on("error:unlock", (e => this.trigger("CallAchievementsUnlockError", e))), this.gp.gamesCollections.on("open", (() => this.trigger("CallGamesCollectionsOpen"))), this.gp.gamesCollections.on("close", (() => {
      ...
      })), this.gp.channels.on("error:deleteMessage", (e => this.trigger("CallOnDeleteMessageError"))), this.gp.triggers.on("activate", (({
        trigger: e
      }) => {
        this.trigger("CallOnTriggerActivate", JSON.stringify(e))
      })), this.gp.triggers.on("claim", (({
        trigger: e
      }) => {
        this.trigger("CallOnTriggerClaim", JSON.stringify(e))
      })), this.gp.triggers.on("error:claim", (e => {
        this.trigger("CallOnTriggerClaimError", JSON.stringify(e))
      })), this.gp.events.on("join", (({
        event: e,
        playerEvent: t
      }) => {
        this.trigger("CallOnEventJoin", JSON.stringify(t))
      })), this.gp.events.on("error:join", (e => {
        this.trigger("CallOnEventJoinError", e)
      })), this.gp.segments.on("enter", (e => {
        this.trigger("CallOnSegmentEnter", e)
      })), this.gp.segments.on("leave", (e => {
        this.trigger("CallOnSegmentLeave", e)
      })), this.gp.schedulers.on("register", (e => {
        this.trigger("CallOnSchedulerRegister", JSON.stringify(e))
      })), this.gp.schedulers.on("error:register", (e => {
        this.trigger("CallOnSchedulerRegisterError", e)
      })), this.gp.schedulers.on("claimDay", (e => {
        this.trigger("CallOnSchedulerClaimDay", JSON.stringify(e))
      })), this.gp.schedulers.on("error:claimDay", (e => {
        this.trigger("CallOnSchedulerClaimDayError", e)
      })), this.gp.schedulers.on("claimDayAdditional", (e => {
        this.trigger("CallOnSchedulerClaimDayAdditional", JSON.stringify(e))
      })), this.gp.schedulers.on("error:claimDayAdditional", (e => {
        this.trigger("CallOnSchedulerClaimDayAdditionalError", e)
      })), this.gp.schedulers.on("claimAllDay", (e => {
        this.trigger("CallOnSchedulerClaimAllDay", JSON.stringify(e))
      })), this.gp.schedulers.on("error:claimAllDay", (e => {
        this.trigger("CallOnSchedulerClaimAllDayError", e)
      })), this.gp.schedulers.on("claimAllDays", (e => {
        this.trigger("CallOnSchedulerClaimAllDays", JSON.stringify(e))
      })), this.gp.schedulers.on("error:claimAllDays", (e => {
        this.trigger("CallOnSchedulerClaimAllDaysError", e)
      })), this.gp.schedulers.on("join", (({
        scheduler: e,
        playerScheduler: t
      }) => {
        this.trigger("CallOnSchedulerJoin", JSON.stringify(t))
      })), this.gp.schedulers.on("error:join", (e => {
        this.trigger("CallOnSchedulerJoinError", e)
      })), this.gp.variables.on("fetchPlatformVariables", (e => {
        this.trigger("CallOnFetchPlatformVariables", JSON.stringify(e))
      })), this.gp.variables.on("error:fetchPlatformVariables", (e => {
        this.trigger("CallOnFetchPlatformVariablesError", e)
      }))
    }
    trigger(e, t) {
    ...
    LeaderboardFetch(e, t, s, r, i, n, a) {
    ...
    LeaderboardScopedOpen(e, t, s, r, i, n, a, l) {
    ...
    LeaderboardScopedFetch(e, t, s, r, i, n, a) {
    ...
    LeaderboardScopedPublishRecord(e, t, s, r, i, n, a, l, h) {
    ...
    AchievementsUnlock(e) {
      ...
      return this.gp.achievements.unlock(s).then((e => {
        e.success ? this.trigger("CallAchievementsUnlock", e.achievement) : this.trigger("CallAchievementsUnlockError")
      })).catch((e => {
    ...
    VariablesFetchPlatformVariables(e) {
      if ("" !== e) {
        for (var t = e.split(",").map((e => e.trim())), s = {}, r = 0; r < t.length; r++) {
          var i = t[r].split(":");
          s[i[0]] = i[1].trim(), console.log(s[i[0]])
        }
        var n = {
          clientParams: s
        };
        this.gp.variables.fetchPlatformVariables(n)
      } else this.gp.variables.fetchPlatformVariables()
    }
    PlayersFetch(e) {
      var t = JSON.parse(e);
      let s = [];
    ...
    Triggers_GetTrigger(e) {
      return this.toUnity(this.gp.triggers.getTrigger(e))
    }
    Triggers_IsActivated(e) {
      return this.toUnity(this.gp.triggers.isActivated(e))
    }
    Triggers_IsClaimed(e) {
      return this.toUnity(this.gp.triggers.isClaimed(e))
    }
    Events_Join(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? {
    ...
    Events_GetEvent(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e;
      return this.toUnity(this.gp.events.getEvent(s).event)
    }
    Events_IsActive(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e,
        r = this.gp.events.has(s);
      return this.toUnity(r)
    }
    Events_IsJoined(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e;
      return this.toUnity(this.gp.events.isJoined(s))
    }
    Segments_List() {
      return this.toUnity(this.gp.segments.list)
    }
    Segments_Has(e) {
      return this.toUnity(this.gp.segments.has(e))
    }
    Experiments_Map() {
    ...
    Rewards_Give(e, t) {
      ...
      this.gp.rewards.give({
        ...r,
        lazy: t
      }).then((e => {
        this.trigger("CallOnRewardsGive", JSON.stringify(e))
      })).catch((e => {
        this.trigger("CallOnRewardsGiveError", e)
      }))
    }
    Rewards_Accept(e) {
      const t = parseInt(e, 10) || 0,
      ...
      this.gp.rewards.accept({
        ...s
      }).then((e => {
        this.trigger("CallOnRewardsAccept", JSON.stringify(e))
      })).catch((e => {
        this.trigger("CallOnRewardsAcceptError", e)
      }))
    }
    Rewards_List() {
    ...
    Rewards_GetReward(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e;
      return this.toUnity(this.gp.rewards.getReward(s))
    }
    Rewards_Has(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e;
      return this.toUnity(this.gp.rewards.has(s))
    }
    Rewards_HasAccepted(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e;
      return this.toUnity(this.gp.rewards.hasAccepted(s))
    }
    Rewards_HasUnaccepted(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e;
      return this.toUnity(this.gp.rewards.hasUnaccepted(s))
    }
    Schedulers_Register(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? {
          id: t
        } : {
      ...
      this.gp.schedulers.register(s)
    }
    Schedulers_ClaimDay(e, t) {
      const s = parseInt(e, 10) || 0,
        r = s > 0 ? s : e;
      console.log(r), console.log(t), this.gp.schedulers.claimDay(r, t)
    }
    Schedulers_ClaimDayAdditional(e, t, s) {
      const r = parseInt(e, 10) || 0,
        i = r > 0 ? r : e,
        n = parseInt(s, 10) || 0,
        a = n > 0 ? n : s;
      console.log(i), console.log(t), console.log(a), this.gp.schedulers.claimDayAdditional(i, t, a)
    }
    Schedulers_ClaimAllDay(e, t) {
      const s = parseInt(e, 10) || 0,
        r = s > 0 ? s : e;
      console.log(r), console.log(t), this.gp.schedulers.claimAllDay(r, t)
    }
    Schedulers_ClaimAllDays(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e;
      console.log(s), this.gp.schedulers.claimAllDays(s)
    }
    Schedulers_List() {
      const e = this.gp.schedulers.list;
      return console.log(e), this.toUnity(e)
    }
    Schedulers_ActiveList() {
      const e = this.gp.schedulers.activeList;
      return console.log(e), this.toUnity(e)
    }
    Schedulers_GetScheduler(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e;
      console.log(s);
      const r = this.toUnity(this.gp.schedulers.getScheduler(s));
      return console.log(r), r
    }
    Schedulers_GetSchedulerDay(e, t) {
      const s = parseInt(e, 10) || 0,
        r = s > 0 ? s : e;
      console.log(r), console.log(t);
      const i = this.toUnity(this.gp.schedulers.getSchedulerDay(r, t));
      return console.log(i), i
    }
    Schedulers_GetSchedulerCurrentDay(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e;
      console.log(s);
      const r = this.toUnity(this.gp.schedulers.getSchedulerCurrentDay(s));
      return console.log(r), r
    }
    Schedulers_IsRegistered(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e,
        r = this.gp.schedulers.isRegistered(s);
      return console.log(r), this.toUnity(r)
    }
    Schedulers_IsTodayRewardClaimed(e) {
      const t = parseInt(e, 10) || 0,
        s = t > 0 ? t : e,
        r = this.gp.schedulers.isTodayRewardClaimed(s);
      return console.log(r), this.toUnity(r)
    }
    Schedulers_CanClaimDay(e, t) {
      const s = parseInt(e, 10) || 0,
        r = s > 0 ? s : e,
        i = this.gp.schedulers.canClaimDay(r, t);
      return console.log(i), this.toUnity(i)
    }
    Schedulers_CanClaimDayAdditional(e, t, s) {
      const r = parseInt(e, 10) || 0,
        i = r > 0 ? r : e,
        n = parseInt(s, 10) || 0,
        a = n > 0 ? n : s,
        l = this.gp.schedulers.canClaimDayAdditional(i, t, a);
      return console.log(l), this.toUnity(l)
    }
    Schedulers_CanClaimAllDay(e, t) {
      const s = parseInt(e, 10) || 0,
        r = s > 0 ? s : e,
        i = this.gp.schedulers.canClaimAllDay(r, t);
      return console.log(i), this.toUnity(i)
    }
    ImagesUpload(e) {
      this.gp.images.upload({
        tags: e.split(",").map((e => e.trim())).filter((e => e))
      }).then((e => {
    ...
    ImagesChooseFile() {
      this.gp.images.chooseFile().then((e => {
        this.trigger("CallImagesChooseFile", e.tempUrl)
      })).catch((e => {
    ...
    ImagesFetch(e) {
      const t = JSON.parse(e);
      this.gp.images.fetch(t).then((e => {
        console.log(e), this.trigger("CallImagesFetchCanLoadMore", e.canLoadMore), this.trigger("CallImagesFetchSuccess", JSON.stringify(e.items))
      })).catch((e => {
    ...
    ImagesFetchMore(e) {
      const t = JSON.parse(e);
      this.gp.images.fetchMore(t).then((e => {
    ...
    ImagesResize(e) {
      const t = JSON.parse(e);
      console.log(t);
      const s = this.gp.images.resize(t.url, t.width, t.height, t.cutBySize);
      this.trigger("CallImagesResize", s)
    }
    CustomCall(e, t) {
      let s = "GamePush." + e;
    ...
    CustomReturn(e, s) {
      let r, i = "GamePush." + e;
      if (null == s) r = window.executeFunctionByName(i, window);
      else {
        let e = (s = s.replace(/\s/g, "")).split(",");
        r = window.executeFunctionByName(i, window, ...e)
      }
      return t(r)
    }
    CustomGetValue(e) {
  ...
  window.executeFunctionByName = function(e, t) {
    ...
    for (var r = e.split("."), i = r.pop(), n = 0; n < r.length; n++) t = t[r[n]];
    try {
      var a = t[i].apply(t, s)
    } catch (e) {
    ...
    for (var s = e.split("."), r = s.pop(), i = 0; i < s.length; i++) t = t[s[i]];
    try {
      var n = t[r]
    } catch (e) {
